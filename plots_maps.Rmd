...
region_df = 
  avo_df %>% 
  mutate(n_obs = n(),sum_volume = sum(total_volume)) %>% 
  mutate(region = str_to_lower(region))

large_region = 
  region_df %>%
  filter(region %in% c("west","midwest","south","southwest","southeast","northeast","midsouth","southCentral","totalus"))
```
```{r}
gdp_df = 
  ...
  select(-1) %>% 
  mutate(
    region = str_remove(Area," "),
    region = str_to_lower(region)
  ) %>% 
  filter(region!="unitedstates") %>% 
  select(-Area) %>% 
  pivot_longer(
    "2013":"2017",
    names_to = "year",
...
avo_year = 
  ...
  summarise(avgp_year = sum(average_price)/n(),
...
avo_gdp = 
  left_join(avo_year, gdp_df, by = c("region","year"))
```
