...
```
Based on the trends plot, we find that the average price of a avocado is different by type, quarter, year, and region. Hence, it is reasonable to consider these four variables as predictors in modeling.
...
## Model comparison
We want to add GDP as another predictor in our model and compare with the previous model. Our new model is:
$$ quarterly \space average \space price = \beta_0 + \beta_1type + \beta_2year + \beta_3quarter + \beta_4 region +\beta_5 gdp$$
